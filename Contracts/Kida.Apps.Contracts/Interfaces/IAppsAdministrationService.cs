...
public interface IAppsAdministrationService
{
    ...
    ValueTask<AppPage> SearchAsync(string? query, AppStatus? status, int page, int pageSize, CancellationToken cancellationToken = default);
    ...
    ValueTask<IFeedback> ChangeAppStatusAsync(Guid appId, AppStatus status, CancellationToken cancellationToken = default);
    ValueTask<IFeedback> ChangeInstallationStatusAsync(Guid installationId, AppStatus status, CancellationToken cancellationToken = default);
}
